...
double precision, dimension (5,5) :: H
...
integer :: i,j,k,n,m, ierr, lwork,ii,jj,kk,numpoints, num, nee, nums
...
do n = 1, num
	do m = 1, num
		...
		H(m,n) = (trap(f2,a,b,nums,L,n,m) - trap(f,a,b,nums,L,n,m))*2*(V0/L)
	end do
H(n,n) = H(n,n) + ((( (hbar**2) * (n**2) * (pi**2)  ) / (2*mass * (L**2))) - ((3.0d0 / 4.0d0) * V0))
...
do ii = 1, numpoints
	...
	psi(jj,ii) = psi(jj,ii) + H(kk,jj) * sqrt(2.0d0/L) *sin((kk*pi*(dble(ii)-1.0d0)*dx)/L)
!print*, psi(jj,ii)
